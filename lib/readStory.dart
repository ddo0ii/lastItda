...
class _ReadStoryState extends State<ReadStory> {
  ...
  Widget _chatList () {
    ...
    return StreamBuilder<QuerySnapshot>(
        stream: Firestore.instance.collection('storyList').document(widget.storyKey).collection("storyChatInfo").snapshots(),
  ...
  Future<String> getLike () async {
    user = await FirebaseAuth.instance.currentUser();
    DocumentReference documentReference =  Firestore.instance.collection("storyList").document(widget.storyKey)
        .collection("storyLikeInfo").document(user.email);
  ...
  Future<void> totalLikeUpdate (int num) async {
    DocumentReference documentReference = await Firestore.instance.collection("storyList").document(widget.storyKey);
  ...
  Future<void> _setChat (String comment) async {
    ...
    DocumentReference documentReference = await Firestore.instance.collection("storyList").document(widget.storyKey)
        .collection("storyChatInfo").document("$index");
